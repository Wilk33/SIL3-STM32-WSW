...
void SL_HandlePCCommand(SL_Context_t *ctx) {
    ...
    if (res.type == CMD_LED_SET && (ctx->state == SL_STATE_IDLE || ctx->state == SL_STATE_STOPPED)) {
...
void SL_HandleMasterResponse(SL_Context_t *ctx, uint8_t masterResponse) {
    ...
    if (ctx->state == SL_STATE_WAIT_AUTH) {
        if (masterResponse == MASTER_GRANT) {
            // Master pozwolił - przechodzimy do wykonania (zapalenia diod w SL_Update)
            // W naszej maszynie stanów SL_Update zobaczy zmianę i wywoła IO_SetLeds
